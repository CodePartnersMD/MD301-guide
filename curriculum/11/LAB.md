...
TODO: Add Google Books API docs
...
TODO: revisit client-side file structure

```sh
book_app_week_3 (organization)
├── book-list-client (client-side repository)
│   ├── data
│   │   └── books.json
|   ├── .eslintrc.json
|   ├── .gitignore
...
│       ├── modules.css
│       └── reset.css
└── book-list-server (server-side repository)
    ├── .eslintrc.json
    ├── .gitignore
    ├── package-lock.json
    ├── package.json
...
TODO: decide where frontend is deployed - AWS S3
TODO: update marketing page to indicate that there will be a charge for AWS
The frontend of your application will be deployed on 
The backend of your application will be deployed on Heroku with a PostgreSQL database provisioned. You and your partner(s) can either clone the database to work with locally or you can create a separate, local database to use during development.
...
- Create a basic HTML scaffold based on the above `book-list-client/` directory structure.
...
- Create a basic server scaffold based on the above `book-list-client/` directory structure.
...
TODO: The following instructions are incorrect; Sam is working on guide issues to resolve this, so come back and add the correct language based on his PR.
...
- Validate that your API has been successfully deployed by sending a GET request to your `/test` endpoint in Postman. You may then remove this route from your server file.

NOTE: steps 4 and 5 were swapped from the original LAB.md
*4. As a developer, I want the client application to have access to a deployed PostgreSQL database so the user data persists across application sessions.*
...
*5. As a developer, I want to seed my local development database with books so I have data to test my development application with.*
...
- Use Postman to request book data from the Google Books API and manually enter each record into your `books` table.
